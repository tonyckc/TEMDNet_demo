...
def model(input, reuse=False, name='nosie_prior', training=True, STDDEV=None):
    with tf.variable_scope(name, reuse=reuse):
        dilated_conv1 = conv_op(input, name + "conv1", 32, training=training, useBN=False, kh=3, kw=3,
                           dh=1, dw=1,
                           padding="SAME", activation=tf.nn.relu, STDDEV=STDDEV)
        dilated_conv2 = conv_op(dilated_conv1, name + "conv2", 64, training=training, useBN=False, kh=3, kw=3,
                           dh=1, dw=1,
                           padding="SAME", activation=tf.nn.relu, STDDEV=STDDEV)
        '''feature learning '''
        ...
        dilated_conv3 = conv_op(res_block_5, name + "conv3", 32, training=training, useBN=False, kh=3, kw=3,
                           dh=1, dw=1,
                           padding="SAME", activation=tf.nn.relu, STDDEV=STDDEV)
        dilated_conv4 = conv_op(dilated_conv3, name + "conv4", 1, training=training, useBN=False, kh=3, kw=3,
                           dh=1, dw=1,
