...
def model(input, reuse=False, name='nosie_prior', training=True, STDDEV=None):
    with tf.variable_scope(name, reuse=reuse):
        ...
        res_block_5 = res_block_layers_v1(res_block_4, 'block_5', [128, 64], change_dimension=True, block_stride=1,
                                          training=training, STDDEV=STDDEV)
        res_1 = tf.add(dilated_conv2, res_block_5)
        dilated_conv3 = dilated_conv_op(res_1, 'dilated_conv3', 32,  training=training, useBN=False, kh=3, kw=3,
                                        rate=2, padding="SAME",
                                        activation=tf.nn.relu, STDDEV=STDDEV)
        res_2 = tf.add(dilated_conv1, dilated_conv3)
        dilated_conv4 = dilated_conv_op(res_2, 'dilated_conv4', 1, training=training, useBN=False, kh=3, kw=3,
                                        rate=1, padding="SAME",
