...
def model(input, reuse=False, name='nosie_prior', training=True, STDDEV=None):
    with tf.variable_scope(name, reuse=reuse):
        ...
        dilated_conv7 = dilated_conv_op(dilated_conv6, 'dilated_conv8', 64, training=training, useBN=True, kh=3, kw=3,
                                        rate=2, padding="SAME",
                                        activation=tf.nn.relu, STDDEV=STDDEV)

        dilated_conv8 = dilated_conv_op(dilated_conv7, 'dilated_conv9', 32, training=training, useBN=True, kh=3, kw=3,
                                        rate=2, padding="SAME",
                                        activation=tf.nn.relu, STDDEV=STDDEV)
        dilated_conv9 = dilated_conv_op(dilated_conv8, 'dilated_conv10', 1, training=training, useBN=True, kh=3, kw=3,
                                         rate=1, padding="SAME",
                                         activation=None, STDDEV=STDDEV)
        result = dilated_conv9
